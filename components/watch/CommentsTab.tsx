...
function CommentCard({ comment, isReply = false, isLight }: { comment: WatchComment; isReply?: boolean; isLight: boolean }) {
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-4">
            ...
            {!isReply && <button className="text-[11px] font-medium transition-colors" style={{ color: muted }} onClick={() => setShowReply((value) => !value)}>{comment.replies?.length ? `${comment.replies.length} ${comment.replies.length === 1 ? 'reply' : 'replies'}` : 'Reply'}</button>}
          </div>
        </div>
      </div>
      <AnimatePresence>{showReply && comment.replies && <motion.div animate={{ height: 'auto', opacity: 1 }} className="overflow-hidden" exit={{ height: 0, opacity: 0 }} initial={{ height: 0, opacity: 0 }}>{comment.replies.map((reply) => <CommentCard comment={reply} isReply isLight={isLight} key={reply.id} />)}</motion.div>}</AnimatePresence>
    </div>
  );
}
...
export default function CommentsTab() {
  ...
  const handlePost = async () => {
    ...
    const next: WatchComment = { id: Date.now(), author: 'You', avatarUrl: contentImages.creators[0].url, text: newComment.trim(), time: 'Just now', likes: 0, badge: { label: 'New Member', color: 'rgb(0,229,186)' } };
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-2"><h3 className="text-base font-bold" style={{ color: title }}>{comments.length} Comments</h3><span className="rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ background: 'rgba(0,229,186,0.1)', color: 'rgb(0,229,186)', border: '1px solid rgba(0,229,186,0.2)' }}>Community</span></div><div className="flex items-center gap-1 rounded-full p-0.5" style={{ background: isLight ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.04)', border: panelBorder }}>{(['top', 'new'] as const).map((option) => <button className="rounded-full px-3 py-1 text-[11px] font-semibold capitalize transition-all" key={option} onClick={() => setSortBy(option)} style={{ background: sortBy === option ? 'rgba(0,229,186,0.15)' : 'transparent', color: sortBy === option ? 'rgb(0,229,186)' : muted }}>{option === 'top' ? 'Top' : 'Newest'}</button>)}</div></div>
      <div className="mb-6 flex gap-3 rounded-xl p-4" style={{ background: panelBg, border: panelBorder }}><div className="relative h-8 w-8 flex-shrink-0 overflow-hidden rounded-full"><Image alt="You" className="object-cover" fill src={contentImages.creators[0].url} /></div><div className="flex-1"><textarea className="w-full resize-none bg-transparent text-xs leading-relaxed outline-none" style={{ color: title }} onChange={(event) => setNewComment(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && (event.metaKey || event.ctrlKey)) handlePost(); }} placeholder="Share your thoughts on this content..." rows={2} value={newComment} /><div className="mt-2 flex items-center justify-between"><span className="text-[10px]" style={{ color: faint }}>Ctrl/Cmd + Enter to post</span><motion.button className="flex items-center gap-1.5 rounded-full px-4 py-1.5 text-[11px] font-semibold transition-all" disabled={!newComment.trim() || isPosting} onClick={handlePost} style={{ background: newComment.trim() ? 'rgba(0,229,186,0.9)' : (isLight ? 'rgba(15,23,42,0.06)' : 'rgba(255,255,255,0.06)'), color: newComment.trim() ? '#0A0F18' : muted }} whileHover={newComment.trim() ? { scale: 1.04 } : {}} whileTap={newComment.trim() ? { scale: 0.96 } : {}}>{isPosting ? <motion.div animate={{ rotate: 360 }} className="h-3 w-3 rounded-full border border-current border-t-transparent" transition={{ duration: 0.7, repeat: Infinity, ease: 'linear' }} /> : <><svg className="h-3 w-3" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" strokeLinecap="round" strokeLinejoin="round" /></svg>Post</>}</motion.button></div></div></div>
      <div className="space-y-5">{sorted.map((comment, index) => <motion.div animate={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 12 }} key={comment.id} transition={{ delay: index * 0.04 }}><CommentCard comment={comment} isLight={isLight} />{index < sorted.length - 1 && <div className="mt-5 border-t" style={{ borderColor: isLight ? 'rgba(15,23,42,0.08)' : 'rgba(255,255,255,0.04)' }} />}</motion.div>)}</div>
      <motion.button className="mt-6 w-full rounded-xl py-3 text-xs font-semibold transition-all" style={{ border: panelBorder, color: muted, background: panelBg }} whileHover={{ backgroundColor: isLight ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.03)' }}>Load more comments</motion.button>
    </div>
  );
}
