...
import { motion, useScroll, useMotionTemplate, AnimatePresence } from 'framer-motion';
...
import { HEADER_HEIGHT, HEADER_NAV_ROW_HEIGHT, HEADER_TOP_ROW_HEIGHT } from '../../lib/config/header';
...
export default function DualLayerHeader() {
  ...
  const currentMainCategory = React.useMemo(() => getMainCategoryFromPath(pathname), [pathname]);

  const headerOpacity = isLight ? 0.96 : 0.9;
  ...
  const headerTopLine = isLight
    ? 'linear-gradient(90deg, rgba(0, 215, 163, 0.28) 0%, rgba(0, 234, 175, 0.24) 45%, rgba(0, 200, 141, 0.28) 100%)'
    : 'linear-gradient(90deg, rgba(0, 159, 116, 0.36) 0%, rgba(0, 234, 175, 0.3) 45%, rgba(0, 200, 141, 0.32) 100%)';
  const progressLine = isLight
    ? 'linear-gradient(90deg, #00cf9d 0%, #00eaaf 52%, #00bf88 100%)'
    : 'linear-gradient(90deg, #00b986 0%, #00eaaf 52%, #00c88d 100%)';
  ...
  React.useEffect(() => {
    if (currentMainCategory !== activeMainCategory) {
      setMainCategory(currentMainCategory);
    }
  }, [activeMainCategory, currentMainCategory, setMainCategory]);
  ...
  const showSubHeader = currentMainCategory !== 'home';
  ...
  return (
    <>
      ...
      >
        <div className="absolute left-0 top-0 h-[3px] w-full" style={{ background: headerTopLine }} />
        <motion.div
          className="absolute left-0 top-0 h-[3px] origin-left"
          style={{ width: '100%', scaleX: scrollYProgress, background: progressLine, boxShadow: '0 1px 8px rgba(0, 229, 186, 0.28)' }}
        />

        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between px-3 sm:px-4 lg:px-5 xl:px-6 w-full gap-2 lg:gap-3" style={{ height: `${HEADER_TOP_ROW_HEIGHT}px` }}>
            <div className="flex min-w-0 flex-1 items-center gap-2 lg:gap-3 overflow-hidden">
              <Link href="/" className="flex items-center gap-3 group">
                ...
                <motion.div className="hidden sm:flex flex-col -space-y-1" whileHover={{ x: 2 }} transition={{ duration: 0.18 }}>
                  ...
                  <span className="text-[10px] text-eco-green/55 font-medium tracking-[0.12em] uppercase">Zero Carbon</span>
                </motion.div>
              </Link>

              <nav className="hidden min-w-0 flex-1 items-center gap-0 overflow-x-auto whitespace-nowrap lg:flex scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
                {mainNavItems.map((item) => (
                  <Link key={item.id} href={item.path}>
                    <motion.button
                      className="relative rounded-full px-1.5 lg:px-2 xl:px-2.5 py-2 text-[11px] lg:text-[11px] xl:text-[12px] font-semibold tracking-[0.002em] transition-all duration-180 whitespace-nowrap"
                      style={{
                        opacity: currentMainCategory === item.id ? 1 : 0.92,
                        color: currentMainCategory === item.id ? '#00eaaf' : navInactiveColor,
                      }}
            ...
            <div className="flex shrink-0 items-center gap-0.5 lg:gap-1">
              ...
              <IconButton ariaLabel={notifCount > 0 ? `Notifications (${notifCount} unread)` : 'Notifications'} background={iconButtonBg} hiddenOnMobile hoverBackground={iconButtonHoverBg} onClick={toggleNotifications}>
                <div className="relative flex items-center justify-center">
                  ...
                  {notifCount > 0 && (
                    <motion.div className="absolute -right-2 -top-2 min-w-[16px] h-4 rounded-full flex items-center justify-center text-[10px] font-black text-white px-0.5" style={{ background: 'rgb(239,68,68)' }} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', damping: 12 }}>
              ...
              <div className="relative hidden sm:block" ref={userMenuRef}>
                <motion.button
                  ...
                  className="flex h-10 items-center gap-2 rounded-full pl-1 pr-2 lg:pr-2.5 transition-all duration-180"
                ...
                >
                  ...
                  <div className="hidden text-left 2xl:block">
                    <div className="max-w-[112px] truncate text-sm font-semibold" style={{ color: isLight ? '#0f172a' : '#f8fafc' }}>
                      {currentUser.name}
                    </div>
                ...
                <AnimatePresence>
                  {userMenuOpen && (
                    <motion.div
                      className="absolute right-0 top-[calc(100%+14px)] z-50 w-[304px] max-w-[calc(100vw-1rem)] sm:w-[320px]"
                    ...
                    >
                      <SurfacePanel
                        ...
                        className="overflow-hidden rounded-[24px] p-0 shadow-2xl"
                      >
                        <div className="max-h-[min(78vh,620px)] overflow-y-auto overscroll-contain">
                        <div className="border-b px-4 py-4 sm:px-5" style={{ borderColor: isLight ? 'rgba(15, 23, 42, 0.08)' : 'rgba(255, 255, 255, 0.08)' }}>
                          <div className="flex items-start gap-3">
                            <div className="flex h-12 w-12 items-center justify-center rounded-2xl text-base font-bold text-white shadow-lg" style={{ background: 'linear-gradient(135deg, rgba(0, 128, 255, 0.88), rgba(0, 229, 186, 0.9))' }}>
                              {userInitial}
                            </div>
                            <div className="min-w-0 flex-1">
                              <p className="truncate text-sm font-semibold sm:text-[15px]" style={{ color: isLight ? '#0f172a' : '#f8fafc' }}>
                                {currentUser.name}
                              </p>
                              <p className="truncate text-[13px]" style={{ color: isLight ? '#475569' : '#94a3b8' }}>
                                {currentUser.email}
                              </p>
                              <div className="mt-2.5 flex flex-wrap gap-2">
                                <span className="rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em]" style={{ background: 'rgba(0, 229, 186, 0.12)', color: '#00eaaf' }}>
                                  {roleLabel}
                                </span>
                                <span className="rounded-full px-2.5 py-1 text-[10px] font-semibold" style={{ background: isLight ? 'rgba(15, 23, 42, 0.06)' : 'rgba(148, 163, 184, 0.16)', color: isLight ? '#334155' : '#cbd5e1' }}>
                        ...
                        <div className="grid grid-cols-2 gap-2.5 px-4 py-3 sm:px-5">
                          <div className="rounded-2xl px-3 py-3" style={{ background: isLight ? 'rgba(0, 234, 175, 0.1)' : 'rgba(0, 234, 175, 0.08)' }}>
                            <p className="text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: '#00eaaf' }}>
                              Carbon Points
                            </p>
                            <p className="mt-1.5 text-lg font-semibold sm:text-[1.7rem]" style={{ color: isLight ? '#0f172a' : '#f8fafc' }}>
                              {currentUser.carbonPoints.toLocaleString()}
                            </p>
                          </div>
                          <div className="rounded-2xl px-3 py-3" style={{ background: isLight ? 'rgba(15, 23, 42, 0.04)' : 'rgba(148, 163, 184, 0.1)' }}>
                            <p className="text-[10px] font-semibold uppercase tracking-[0.14em]" style={{ color: isLight ? '#475569' : '#94a3b8' }}>
                              Status
                            </p>
                            <p className="mt-1.5 text-lg font-semibold sm:text-[1.7rem]" style={{ color: isLight ? '#0f172a' : '#f8fafc' }}>
                              Active
                        ...
                        <div className="space-y-2 px-4 pb-4 sm:px-5">
                          {accountSummary.map((item) => (
                            ...
                            >
                              <span className="text-[13px] font-medium" style={{ color: isLight ? '#475569' : '#94a3b8' }}>
                                {item.label}
                              </span>
                              <span className="text-[13px] font-semibold" style={{ color: isLight ? '#0f172a' : '#f8fafc' }}>
                                {item.value}
                              </span>
                            </div>
                          ))}
                        ...
                        <div className="border-t px-4 py-4 sm:px-5" style={{ borderColor: isLight ? 'rgba(15, 23, 42, 0.08)' : 'rgba(255, 255, 255, 0.08)' }}>
                          <div className="grid gap-2">
                            <Link href="/dashboard" onClick={() => setUserMenuOpen(false)}>
                              <div className="flex items-center justify-between rounded-2xl px-4 py-2.5 transition-colors" style={{ background: isLight ? 'rgba(0, 234, 175, 0.08)' : 'rgba(0, 234, 175, 0.07)' }}>
                                <span className="text-[13px] font-semibold" style={{ color: isLight ? '#0f172a' : '#f8fafc' }}>
                                  Dashboard
                                </span>
                                <span className="text-[11px] font-semibold uppercase tracking-[0.12em]" style={{ color: '#00eaaf' }}>
                                  Open
                                </span>
                              </div>
                            </Link>
                            <Link href="/settings/security" onClick={() => setUserMenuOpen(false)}>
                              <div className="flex items-center justify-between rounded-2xl px-4 py-2.5 transition-colors" style={{ background: isLight ? 'rgba(15, 23, 42, 0.04)' : 'rgba(15, 23, 42, 0.7)' }}>
                                <span className="text-[13px] font-semibold" style={{ color: isLight ? '#0f172a' : '#f8fafc' }}>
                                  Security Settings
                                </span>
                                <span className="text-[11px] font-semibold uppercase tracking-[0.12em]" style={{ color: isLight ? '#475569' : '#94a3b8' }}>
                                  Manage
                                </span>
                              </div>
                            </Link>
                            <button
                              className="flex items-center justify-between rounded-2xl px-4 py-2.5 text-left transition-colors"
                              onClick={() => {
                                setUserMenuOpen(false);
                              ...
                              type="button"
                            >
                              <span className="text-[13px] font-semibold" style={{ color: isLight ? '#991b1b' : '#fecaca' }}>
                                Sign Out
                              </span>
                              <span className="text-[11px] font-semibold uppercase tracking-[0.12em]" style={{ color: isLight ? '#b91c1c' : '#fca5a5' }}>
                                Exit
                              </span>
                            </button>
                          </div>
