...
export default function HomeCtaSection({ badge, body, primaryLabel, secondaryLabel, titlePrefix, highlight }: HomeCtaSectionProps) {
  return (
    <section className="relative overflow-hidden py-20">
      ...
      <motion.div className="relative z-10 mx-auto max-w-2xl px-6 text-center" initial={{ opacity: 0, y: 20 }} viewport={{ once: true }} whileInView={{ opacity: 1, y: 0 }}>
        ...
        <h2 className="mb-4 text-4xl font-black leading-tight" style={{ color: 'var(--app-title-color)' }}>
          {titlePrefix}{' '}
          <span className="bg-gradient-to-r from-eco-green-bright to-eco-green bg-clip-text text-transparent">{highlight}</span>
        </h2>
        <p className="mb-8 text-sm leading-relaxed" style={{ color: 'var(--app-body-color)' }}>{body}</p>
